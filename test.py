...
# Define BLEU score weights for different n-gram levels
w1 = (1.0, 0.0, 0.0, 0.0)  # BLEU-1: only 1-grams
w2 = (0.5, 0.5, 0.0, 0.0)  # BLEU-2: 1-grams and 2-grams
w3 = (0.33, 0.33, 0.33, 0.0)  # BLEU-3: 1-grams, 2-grams, and 3-grams  
w4 = (0.25, 0.25, 0.25, 0.25)  # BLEU-4: all n-grams up to 4-grams

def calculate_bleu_evaluation(GT_sentences, predicted_sentences):
    # Convert sentences to list of tokens for BLEU computation
    # GT_sentences should be a list of lists (reference sentences)
    # predicted_sentences should be a list of lists (candidate sentences)
    GT_tokenized = [[sent.split()] for sent in GT_sentences]
    pred_tokenized = [sent.split() for sent in predicted_sentences]
    
    BLEU_1 = corpus_bleu(GT_tokenized, pred_tokenized, weights=w1)
    BLEU_2 = corpus_bleu(GT_tokenized, pred_tokenized, weights=w2)
    BLEU_3 = corpus_bleu(GT_tokenized, pred_tokenized, weights=w3)
    BLEU_4 = corpus_bleu(GT_tokenized, pred_tokenized, weights=w4)

    return BLEU_1, BLEU_2, BLEU_3, BLEU_4

bleu1_score, bleu2_score, bleu3_score, bleu4_score = calculate_bleu_evaluation(test_cap, gen_cap)
